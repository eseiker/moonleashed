/*
 * AES-CMAC for NimBLE's Security Manager, without mbedtls' generic cipher layer
 * (TASK-705).
 *
 * NimBLE's ble_sm_alg.c computes f4, f5, f6 and g2 with mbedtls_cipher_cmac,
 * which drags in cipher.c, cipher_wrap.c and cmac.c — a generic algorithm
 * dispatch table used for exactly one fixed algorithm, AES-128. That costs
 * about 2 KB of flash, and the firmware needs a little under 4 KB more free to
 * reach the 64 KB minimum an XIP region requires (KNOW-693).
 *
 * ble_sm_alg_aes_cmac is static inside upstream's file, so it cannot be
 * replaced by renaming a symbol the way the P-256 backend was (TASK-698).
 * Instead this file provides mbedtls_cipher_cmac itself, implemented straight
 * over mbedtls_aes_crypt_ecb, and lib/mbedtls.scons stops building the cipher
 * layer. Nothing else in the firmware uses it: NFC calls mbedtls_aes_* and
 * mbedtls_des_* directly.
 *
 * The algorithm is RFC 4493. Only AES-128 is supported, which is all the
 * Security Manager asks for.
 */
...
/* A stand-in for mbedtls' cipher descriptor. ble_sm_alg.c only passes the
 * result of mbedtls_cipher_info_from_type straight into mbedtls_cipher_cmac,
 * and both ends are in this file, so the value only has to be non-NULL. */
...
/* Double a 128-bit value in GF(2^128), the subkey step of RFC 4493. */
static void cmac_double(uint8_t* b) {
    ...
    b[CMAC_BLOCK - 1] <<= 1;
    /* The reduction polynomial for a 128-bit block is 0x87. */
...
int mbedtls_cipher_cmac(
    ...
    if(keylen != 128) return -1; /* AES-128 only */
    ...
    if(rc == 0) {
        /* Subkeys: L = AES(key, 0), K1 = L<<1, K2 = K1<<1. */
    ...
    if(rc == 0) {
        ...
        if(rc == 0) {
            /* Last block: whole blocks use K1, a partial or empty one is
             * padded with 0x80 then zeros and uses K2. */
...
bool sm_cmac_selftest(void) {
    /* RFC 4493 test vectors, all with the same key. */
