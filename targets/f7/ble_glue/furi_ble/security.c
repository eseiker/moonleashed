...
void ble_security_deinit(void) {
    ...
    sm_glue_oob_clear(); /* also clears the OOB flag in the pairing request */
...
bool ble_security_configure(
    ...
    if(secure_connections && !sm_glue_sc_supported()) {
        FURI_LOG_E(TAG, "secure connections requested but not built in");
        return false;
    }
    sm_glue_configure(
